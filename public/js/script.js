...
// ESG section
    ...
    try {
      ...
      if (url.origin !== location.origin) link.removeAttribute('download');
		...
		const modal = document.getElementById('sanamModal');
		const closeBtn = document.getElementById('sanamModalClose');
		const backdrop = modal ? modal.querySelector('.sanam-modal-backdrop') : null;

		if (!modal) return;

		const STORAGE_KEY = 'sanam_home_modal_seen';
